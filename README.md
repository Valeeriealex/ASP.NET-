...
это процесс проверки подлинности пользователя, чтобы убедиться, что он действительно тот, за кого
себя выдает. Это может включать в себя проверку учетных данных, таких как логин и пароль, или
использование более безопасных методов, таких как токены.
Цель аутентификации – установить, что пользователь – это тот, за кого он себя выдает. После успешной
аутентификации пользователь обычно получает токен или другой идентификатор, который
предоставляет доступ к ресурсам.
* Авторизация (Authorization):
это процесс определения прав доступа пользователя к определенным ресурсам или действиям. Она
определяет, какие действия пользователь может выполнять после успешной аутентификации.

## **JWT**
JWT (JSON Web Token) – это стандарт для представления утверждений (claims) в виде структурированных
JSON-объектов.  
* Заголовок (Header): Заголовок обычно состоит из двух частей: типа токена (JWT) и алгоритма,
используемого для подписи токена.
* Полезная нагрузка (Payload): Этот раздел содержит утверждения (claims). Утверждения представляют
информацию о пользователе и дополнительные метаданные. Существуют три типа утверждений:
утверждения о претензиях (Claims), утверждения об области применения (Scope), и утверждения о
ролях (Roles).
* Подпись (Signature): Подпись создается на основе заголовка, утверждений и секретного ключа. Это
обеспечивает целостность данных и аутентификацию токена.

## **Postman**
Инструмент тестирования, позволяющий делать запросы к вашему API в удобной форме. 

## **OAuth**
OAuth (Open Authorization) – это протокол аутентификации и авторизации, который позволяет приложениям
запрашивать доступ к ресурсам от имени пользователя без необходимости знания его учетных данных. 
